...
5. **DeepSeek 中转 Claude Opus 4.7 thinking 模式报错**: DeepSeek 中转 Claude Opus 4.7 时，thinking 内容需要在每轮请求中回传（`content[].thinking` 字段），否则 API 会报 "缺少 thinking 回传" 错误。Hermes Agent 不兼容此模式，**最简单的解决办法是直接关闭 thinking/推理模式**：
   - 方式一（推荐）：`hermes config set model.reasoning_effort none` — 永久关闭
   - 方式二：会话中执行 `/reasoning none` — 仅当前会话
   - 注意：如果通过 OpenRouter 调用相同模型则不会有此问题，这是 DeepSeek 中转层的特殊要求。
